...
class security:
    ...
    def credit_card_check(in_packet):
        visa_num = findall('4[0-9]{12}(?:[0-9]{3})?', str(raw))
        mastercard_num = findall('(?:5[1-5][0-9]{2}|222[1-9]|22[3-9][0-9]|2[3-6][0-9]{2}|27[01][0-9]|2720)[0-9]{12}', str(raw))
        diners_club_num = findall('3(?:0[0-5]|[68][0-9])[0-9]{11}', str(raw))
        discover_num = findall('6(?:011|5[0-9]{2})[0-9]{12}', str(raw))
        jcb_num = findall('(?:2131|1800|35\d{3})\d{11}', str(raw))
        american_express_num = findall('3[47][0-9]{13}', str(raw))
        bcglobal_num = findall('^(6541|6556)[0-9]{12}', str(raw))
        korean_local_num = findall('^9[0-9]{15}', str(raw))
        laser_card_num = findall('^(6304|6706|6709|6771)[0-9]{12,15}', str(raw))
        maestro_num = findall('^(5018|5020|5038|6304|6759|6761|6763)[0-9]{8,15}', str(raw))
        union_pay_num = findall('^(62[0-9]{14,17})', str(raw))
